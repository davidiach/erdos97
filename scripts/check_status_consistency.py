...
BANNED_OVERCLAIM_RE_LIST = (
    re.compile(
        rf"\b(?:proved|proven|settled|solved|disproved)\b[^.\n]{{0,120}}\b{ERDOS97_TARGET_RE}\b",
        re.I,
    ),
    re.compile(
        rf"\b{ERDOS97_TARGET_RE}\b[^.\n]{{0,120}}\b(?:proved|proven|settled|solved|disproved)\b",
        re.I,
    ),
    re.compile(
...
def find_forbidden_overclaim_lines(text: str) -> list[tuple[int, str]]:
    ...
    for index, line in enumerate(text.splitlines(), start=1):
        ...
        if not normalized or OVERCLAIM_ALLOW_RE.search(normalized):
            continue
        if any(pattern.search(normalized) for pattern in BANNED_OVERCLAIM_RE_LIST):
