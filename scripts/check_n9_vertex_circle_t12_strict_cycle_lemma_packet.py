...
def _validate_local_lemma(packet: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(selected_equalities, list) or len(selected_equalities) != EXPECTED_CYCLE_LENGTH:
        ...
        try:
            ...
            errors.append(f"F16 local_lemma selected_distance_equalities unsupported: {exc}")
        else:
            expect_equal(
                errors,
                "F16 local_lemma selected_distance_equalities",
                selected_equalities,
                expected_steps,
            )
...
def _validate_cycle_step_support(
    ...
    *,
    ...
    if not isinstance(rows, list):
        errors.append("F16 core_selected_rows must be a list")
        return
    if not isinstance(steps, list):
        errors.append("F16 cycle_steps must be a list")
        return
    if len(steps) != EXPECTED_CYCLE_LENGTH:
        errors.append("F16 cycle_steps must have three entries")
        return
    for index, step in enumerate(steps):
        if not isinstance(step, dict):
            errors.append(f"F16 cycle_steps[{index}] must be an object")
            continue
        strict = step.get("strict_inequality")
        equality = step.get("equality_to_next_outer_pair")
        if not isinstance(strict, dict):
            errors.append(f"F16 cycle_steps[{index}] strict_inequality must be an object")
            continue
        if not isinstance(equality, dict):
            errors.append(
                f"F16 cycle_steps[{index}] equality_to_next_outer_pair must be an object"
            )
            continue
        try:
            ...
            errors.append(
                f"F16 cycle step {index} strict_inequality is not supported by "
                f"selected rows: {exc}"
            )
        try:
            ...
            errors.append(
                f"F16 cycle step {index} equality connector is not supported by "
                f"selected rows: {exc}"
            )
        try:
            ...
            if pair(*equality["start_pair"]) != pair(*strict["inner_pair"]):
                errors.append(
                    f"F16 cycle step {index} equality must start at current inner pair"
                )
            if pair(*equality["end_pair"]) != pair(*next_strict["outer_pair"]):
                errors.append(
                    f"F16 cycle step {index} equality must end at next outer pair"
                )
        except (KeyError, TypeError, ValueError) as exc:
            errors.append(f"F16 cycle step {index} closure validation failed: {exc}")
    try:
        expect_equal(
            errors,
            "F16 cycle_pair_chain",
            packet.get("cycle_pair_chain"),
            _expected_cycle_pair_chain(steps),
        )
    except (KeyError, TypeError, ValueError) as exc:
        errors.append(f"F16 cycle_pair_chain unsupported: {exc}")
