#!/usr/bin/env python3
"""Generate or check the exact large-halo slot-budget packet."""
...
DEFAULT_ARTIFACT = (
    ROOT / "data" / "certificates" / "fragile_cycle_halo_slot_budget.json"
)
...
def _census_summary(section: dict[str, Any]) -> dict[str, Any]:
    return {
        "halo_count": section["halo_count"],
        "placement_count": section["placement_count"],
        "minimum_retained_private_halos": section[
            "minimum_retained_private_halos"
        ],
        "aggregate_counts": section["aggregate_counts"],
        "retained_private_halo_histogram": section[
            "retained_private_halo_histogram"
        ],
        "spare_kind_histogram": section["spare_kind_histogram"],
        "placement_trace_sha256": section["placement_trace_sha256"],
    }
...
def summary_json_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "slot_budget_lemma": payload["slot_budget_lemma"],
        "four_halos": _census_summary(payload["four_halos"]),
        "five_halos": _census_summary(payload["five_halos"]),
...
def main(argv: list[str] | None = None) -> int:
    ...
    start = perf_counter()
    generated = halo_slot_budget_payload()
    if args.check:
    ...
    if args.summary_json:
        ...
        print("exact fragile-cycle halo slot budget")
        print(
            "four-halo retained-private histogram: "
            f"{payload['four_halos']['retained_private_halo_histogram']}"
        )
        print(
            "five-halo retained-private histogram: "
            f"{payload['five_halos']['retained_private_halo_histogram']}"
        )
