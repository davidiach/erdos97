#!/usr/bin/env python3
"""Generate or check the focused T05/F10 n=9 self-edge local lemma packet."""
...
from erdos97.n9_vertex_circle_t05_self_edge_lemma_packet import (  # noqa: E402
    CLAIM_SCOPE,
    ...
    source_artifacts,
    t05_self_edge_lemma_packet_payload,
)
...
def load_source_payloads(
    *,
    ...
    """Load the source artifacts used by the focused T05 packet."""
...
def _expected_payload(
    ...
    try:
        return t05_self_edge_lemma_packet_payload(
            source_payloads["self_edge_packet"],
            source_payloads["template_catalog"],
        )
    except (AssertionError, KeyError, TypeError, ValueError) as exc:
        errors.append(f"source-bound T05 self-edge lemma packet failed: {exc}")
        return None
...
def validate_payload(
    ...
    *,
    ...
    """Return validation errors for a focused T05/F10 local lemma packet."""
    ...
    try:
        ...
        errors.append(f"expected T05 self-edge lemma packet counts failed: {exc}")

    if recompute and expected_payload is not None and not errors:
        expect_equal(errors, "T05 self-edge lemma packet", payload, expected_payload)
    return errors
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.write:
        payload = t05_self_edge_lemma_packet_payload(
    ...
    if args.json:
        ...
        print("n=9 vertex-circle T05/F10 self-edge local lemma packet")
        ...
        if args.check or args.assert_expected:
            print("OK: T05/F10 self-edge local lemma packet checks passed")
