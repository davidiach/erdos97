#!/usr/bin/env python3
"""Check the bootstrap/T12 81:3 first-supply-chain prefix CSP."""
...
from erdos97.bootstrap_t12_81_3_first_supply_chains import (  # noqa: E402
    DEFAULT_ARTIFACT,
...
def print_summary(payload: dict[str, object]) -> None:
    ...
    print("bootstrap / T12 81:3 first-supply-chain prefix CSP")
    print(
        "implicit selected assignment space: "
        f"{summary['implicit_selected_assignment_space_size']}"
    )
    print(f"first-step support pairs: {summary['fixed_prefix_support_pair_count']}")
    print(f"search nodes: {summary['search_node_count']}")
    print(f"survivors: {summary['surviving_prefix_pair_count']}")
...
def main() -> int:
    ...
    generated = build_t12_81_3_first_supply_chains_payload()
    payload = generated
    ...
    if args.json:
        ...
        if args.check:
            print("OK: stored bootstrap/T12 81:3 first-supply-chain artifact matches")
        if args.assert_expected:
            print("OK: bootstrap/T12 81:3 first-supply-chain expectations verified")
    return 0
