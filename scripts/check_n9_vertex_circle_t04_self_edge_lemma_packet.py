#!/usr/bin/env python3
"""Generate or check the focused T04/F13 n=9 self-edge local lemma packet."""
...
from erdos97.n9_vertex_circle_t04_self_edge_lemma_packet import (  # noqa: E402
    ...
    source_artifacts,
    t04_self_edge_lemma_packet_payload,
)
...
def load_source_payloads(
    *,
    ...
    """Load the source artifacts used by the focused T04 packet."""
...
def _expected_payload(
    ...
    try:
        return t04_self_edge_lemma_packet_payload(
            source_payloads["self_edge_packet"],
            source_payloads["template_catalog"],
        )
    except (AssertionError, KeyError, TypeError, ValueError) as exc:
        errors.append(f"source-bound T04 self-edge lemma packet failed: {exc}")
        return None
...
def validate_payload(
    ...
    *,
    ...
    """Return validation errors for a focused T04/F13 local lemma packet."""
    ...
    try:
        ...
        errors.append(f"expected T04 self-edge lemma packet counts failed: {exc}")

    if recompute and expected_payload is not None and not errors:
        expect_equal(errors, "T04 self-edge lemma packet", payload, expected_payload)
    return errors
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.write:
        payload = t04_self_edge_lemma_packet_payload(
    ...
    if args.json:
        ...
        print("n=9 vertex-circle T04/F13 self-edge local lemma packet")
        ...
        if args.check or args.assert_expected:
            print("OK: T04/F13 self-edge local lemma packet checks passed")
