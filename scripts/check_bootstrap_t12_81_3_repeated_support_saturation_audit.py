#!/usr/bin/env python3
"""Check the bootstrap/T12 81:3 repeated-support saturation audit."""
...
from erdos97.bootstrap_t12_81_3_repeated_support_saturation_audit import (  # noqa: E402
    DEFAULT_ARTIFACT,
...
def print_summary(payload: dict[str, object]) -> None:
    ...
    by_level = summary["unique_catalogue_count_by_repeated_support_count"]
    if not isinstance(by_level, dict):
        raise AssertionError("level counts must be a mapping")
    print("bootstrap / T12 81:3 repeated-support saturation audit")
    print(f"source prefix survivors: {summary['source_prefix_survivor_count']}")
    print(f"one-repeated catalogues: {by_level['1']}")
    print(f"two-repeated catalogues: {by_level['2']}")
    print(f"three-repeated catalogues: {by_level['3']}")
    print(f"terminal repeated-support catalogues: {summary['terminal_catalogue_count']}")
...
def main() -> int:
    ...
    generated = build_t12_81_3_repeated_support_saturation_audit_payload()
    payload = generated
    ...
    if args.summary_json:
        ...
        if args.check:
            print("OK: stored bootstrap/T12 81:3 saturation artifact matches")
        if args.assert_expected:
            print("OK: bootstrap/T12 81:3 saturation expectations verified")
    return 0
