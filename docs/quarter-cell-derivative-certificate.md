...
## Follow-up: all-m closure of the three mixed-derivative cells

The three signed cells whose proof rule is `F(d,0) = F(0,e) = 0 and
F_de < 0` (`LL_y-_z+`, `LH_y+_z+`, `HH_y+_z-`) are now closed for all
...
polynomial relaxation containing every `T in (0, pi/4]`. For those three
cells this artifact is superseded as the primary closure and retained as an
independent finite-`m` cross-check; for the other nine signed cells this
artifact remains the only machine closure, at `m = 8, 12, 16`.
