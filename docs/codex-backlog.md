...
The missing bridge is now structural and metric: force the `23=27` core from
a genuine minimal counterexample; in the triggered branch, force or exploit
the endpoint-reuse/T5/T44 alternative; in the pair-free branch, use
critical-radius, full-extension, or ordinary-distance geometry beyond the
current deletion-coverage lemmas.
...
4. Use the bootstrap-core crosswalk and the bootstrap / vertex-circle overlay
   ...
   vertex-circle replay. The target-sparse repair-extension command
   ...
   candidates still fail basic filters before vertex-circle replay. The
   support-cone command
   `python scripts/check_bootstrap_t12_151_6_label4_target_sparse_support_cone.py --check --assert-expected --json`
   converts the cascade supports center `5` with `[4,6]` and center `6` with
   `[0,5]` into exact quotient equalities. The local target-pair and completion
   probes have no bounded one- or two-row Kalmanson/Altman cone certificate;
...
5. Extend the Kalmanson template diagnostics toward order-search coverage:
   ...
   wide residual certificates before its configured limit. The completed
   compression in
   `docs/sparse-full-cone-c25-transfer-residual-compression.md` reduces all
