# n=9 T06/F11 Self-edge Mini-replay

Status: `REVIEW_PENDING_DIAGNOSTIC`.

This mini-replay is a deliberately small verifier for the focused T06/F11
...
- the stored T06 family packet `F11`;
...
```bash
data/certificates/n9_t06_self_edge_minireplay.json
```

Generate and check:

```bash
python scripts/check_n9_t06_self_edge_minireplay.py --write --assert-expected
...
```bash
python -m pytest tests/test_n9_t06_self_edge_minireplay.py -q
```

## Interpretation

The replay confirms that the stored T06/F11 packet supports the local
contradiction shape:
...
This covers only the exact T06/F11 family packet listed above. It is
