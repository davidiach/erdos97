...
`C13_sidon_1_2_4_10` are now exactly killed across all cyclic orders by
...
tested C29 fixed order. That C29 order is now superseded by
`data/certificates/c29_sidon_fixed_order_kalmanson_165_unsat.json`, which kills
the same fixed order by a full Kalmanson/Farkas certificate. The minimum-radius
