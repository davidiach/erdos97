# n=9 T02 Self-edge Mini-replay

Status: `REVIEW_PENDING_DIAGNOSTIC`.

This mini-replay is a deliberately small verifier for the focused T02
...
- the four stored T02 family packets `F01`, `F04`, `F08`, and `F14`;
- each family's three local selected rows;
...
```bash
data/certificates/n9_t02_self_edge_minireplay.json
```

Generate and check:

```bash
python scripts/check_n9_t02_self_edge_minireplay.py --write --assert-expected
...
```bash
python -m pytest tests/test_n9_t02_self_edge_minireplay.py -q
```

## Interpretation

The replay confirms that each stored T02 family packet supports the local
contradiction shape:
...
This covers only the exact T02 family packets listed above. It is proof-mining
