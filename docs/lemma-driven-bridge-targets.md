...
- Two-vertex deletion classifies every certifying center's complete rich
  profile as `T4`, `T5`, or `T44`.  Counting every rich class simultaneously
  gives global, localized, and exclusive-endpoint capacity bounds, including
  exact restrictions on the three-nonendpoint blocks of exclusive endpoint
  rows.  See `docs/minimal-two-deletion-profile.md` and
