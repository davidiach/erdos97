# n=9 T05/F10 Self-edge Mini-replay

Status: `REVIEW_PENDING_DIAGNOSTIC`.

This mini-replay is a deliberately small verifier for the focused T05/F10
...
- the stored T05 family packet `F10`;
...
```bash
data/certificates/n9_t05_self_edge_minireplay.json
```

Generate and check:

```bash
python scripts/check_n9_t05_self_edge_minireplay.py --write --assert-expected
...
The replay confirms that the stored T05/F10 packet supports the local
contradiction shape:
...
This covers only the exact T05/F10 family packet listed above. It is
