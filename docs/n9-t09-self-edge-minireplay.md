# n=9 T09/F03 Self-edge Mini-replay

Status: `REVIEW_PENDING_DIAGNOSTIC`.

This mini-replay is a deliberately small verifier for the focused T09/F03
...
- the stored T09 family packet `F03`;
- its six local selected rows;
- its selected-distance equality chain of length `6`;
...
```bash
data/certificates/n9_t09_self_edge_minireplay.json
```

Generate and check:

```bash
python scripts/check_n9_t09_self_edge_minireplay.py --write --assert-expected
...
```bash
python -m pytest tests/test_n9_t09_self_edge_minireplay.py -q
```

## Interpretation

The replay confirms that the stored T09/F03 packet supports the local
contradiction shape:
...
This covers only the exact T09/F03 family packet listed above. It is
