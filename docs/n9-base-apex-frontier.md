...
The P19 degree obstruction in
`scripts/check_n9_base_apex_d3_p19_degree_obstruction.py` closes the first
profile ledger of that packet, rows `R000` through `R007`, by incident-degree
congruence. For the P19 profile multiset `[0,0,0,0,0,0,0,0,6]`, every
profile-capacity assignment contributes incident counts divisible by `3` at
each vertex, but a D=3 row subtracts endpoint degrees from the full nonagon
capacity `14`. Three deficient chords have total endpoint degree `6`, so the
required vertexwise residues cannot all vanish modulo `3`. This reduces only
the finite D=3 packet target from `88` rows to `80`; it is not a proof of
`n=9`, not incidence completeness, and not a geometric realizability test. See
`docs/n9-base-apex-d3-p19-degree-obstruction.md`.

The P20 residue obstruction in
`scripts/check_n9_base_apex_d3_p20_residue_obstruction.py` closes the next
profile ledger, rows `R008` through `R015`, by a mod-3 residue-support count.
For the P20 profile multiset `[0,0,0,0,0,0,0,1,5]`, the profile side can create
incident residue `2` at at most two vertices, while any D=3 row leaves at least
three vertices untouched by the deficient chords and hence needing target
residue `14 mod 3 = 2`. Together with the P19 degree obstruction, this reduces
only the finite D=3 packet target from `88` rows to `72`; it is not a proof of
`n=9`, not incidence completeness, and not a geometric realizability test. See
