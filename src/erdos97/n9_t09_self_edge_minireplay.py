"""Minimal replay for the n=9 T09/F03 self-edge local lemma packet.

This module treats the focused T09 packet as input data and checks only the
small local proof skeleton for its F03 family packet: selected-distance
...
SCHEMA = "erdos97.n9_t09_self_edge_minireplay.v1"
STATUS = "REVIEW_PENDING_T09_SELF_EDGE_MINIREPLAY"
TRUST = "REVIEW_PENDING_DIAGNOSTIC"
SOURCE_PACKET_SCHEMA = "erdos97.n9_vertex_circle_t09_self_edge_lemma_packet.v1"
SOURCE_PACKET = "data/certificates/n9_vertex_circle_t09_self_edge_lemma_packet.json"

EXPECTED_FAMILY_IDS = ["F03"]
EXPECTED_ASSIGNMENT_COUNT = 18
EXPECTED_ASSIGNMENT_COUNTS = {"F03": 18}
EXPECTED_STRICT_ROWS = {"F03": 0}
EXPECTED_OUTER_PAIRS = {"F03": [1, 3]}
EXPECTED_INNER_PAIRS = {"F03": [1, 2]}


def replay_family(packet: dict[str, Any]) -> tuple[dict[str, object], list[str]]:
    """Replay the T09/F03 family packet and return a compact summary plus errors."""
    errors: list[str] = []
...
def replay_packet(packet: dict[str, Any]) -> tuple[dict[str, object], list[str]]:
    ...
    if sum(assignment_counts.values()) != EXPECTED_ASSIGNMENT_COUNT:
        errors.append("family assignment counts do not sum to expected T09 count")
    ...
    if not summary["all_self_edge_contradictions"]:
        errors.append("not every T09 family replay closed a self-edge contradiction")
    return summary, errors
...
def minireplay_payload(packet: dict[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "replay": replay,
        "interpretation": (
            "The T09/F03 family packet identifies the strict outer and inner "
...
def assert_expected_payload(payload: dict[str, Any]) -> None:
    """Assert the stable expected T09/F03 mini-replay facts."""
