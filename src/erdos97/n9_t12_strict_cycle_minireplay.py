"""Minimal replay for the n=9 T12 strict-cycle local lemma packet.

This module treats the focused T12 packet as input data and checks only the
...
SCHEMA = "erdos97.n9_t12_strict_cycle_minireplay.v1"
STATUS = "REVIEW_PENDING_T12_STRICT_CYCLE_MINIREPLAY"
...
def replay_packet(packet: dict[str, Any]) -> tuple[dict[str, object], list[str]]:
    """Replay the local T12 packet and return a compact summary plus errors."""
    errors: list[str] = []
...
def minireplay_payload(packet: dict[str, Any]) -> dict[str, object]:
    """Build the stable T12 mini-replay artifact payload."""
...
def assert_expected_payload(payload: dict[str, Any]) -> None:
    """Assert the stable expected T12 mini-replay facts."""
