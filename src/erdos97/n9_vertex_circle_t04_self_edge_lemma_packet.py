"""Build a focused T04/F13 n=9 self-edge local lemma packet.
...
CLAIM_SCOPE = (
    "Focused T04/F13 self-edge local lemma packet for two n=9 frontier "
...
EXPECTED_ASSIGNMENT_IDS = ("A023", "A174")
...
EXPECTED_FAMILY_ASSIGNMENT_COUNTS = {"F13": 2}
EXPECTED_FAMILY_ORBIT_SIZES = {"F13": 2}
EXPECTED_PATH_LENGTH_COUNTS = {"3": 2}
EXPECTED_SHARED_ENDPOINT_COUNTS = {"1": 2}
EXPECTED_SELECTED_PATH_SHAPE_COUNTS = {"2:1:1:path=3": 2}
...
def _family_records_by_id(template: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if missing:
        raise ValueError(f"missing T04 families: {missing!r}")
...
def source_artifacts(
    ...
    """Return embedded source-artifact metadata for the T04 packet."""

    return [
        {
            "path": "data/certificates/n9_vertex_circle_self_edge_template_packet.json",
            "role": "source T04/F13 self-edge template record",
        ...
        },
        {
            ...
            "role": "catalog crosswalk confirming T04 coverage and shape summary",
...
def _primary_conflict(replay: dict[str, Any], strict: Mapping[str, Any]) -> dict[str, Any]:
    ...
    raise AssertionError("primary T04 self-edge conflict not found in replay")
...
def _assert_source_records(
    ...
    if template["template_key"] != EXPECTED_TEMPLATE_KEY:
        raise AssertionError("unexpected T04 template key")
    if template["status"] != "self_edge":
        raise AssertionError("T04 must remain a self-edge template")
    if template["assignment_ids"] != list(EXPECTED_ASSIGNMENT_IDS):
        raise AssertionError("unexpected T04 assignment ids")
    if template["assignment_count"] != EXPECTED_ASSIGNMENT_COUNT:
        raise AssertionError("unexpected T04 assignment count")
    if template["families"] != list(EXPECTED_FAMILY_IDS):
        raise AssertionError("unexpected T04 family list")
    if template["family_count"] != EXPECTED_FAMILY_COUNT:
        raise AssertionError("unexpected T04 family count")
    if template["orbit_size_sum"] != EXPECTED_ORBIT_SIZE_SUM:
        raise AssertionError("unexpected T04 orbit-size sum")
    if template["core_size"] != EXPECTED_CORE_SIZE:
        raise AssertionError("unexpected T04 core size")
    if template["strict_edge_count"] != EXPECTED_STRICT_EDGE_COUNT:
        raise AssertionError("unexpected T04 strict-edge count")
    if template["path_length_counts"] != EXPECTED_PATH_LENGTH_COUNTS:
        raise AssertionError("unexpected T04 path-length counts")
    if template["shared_endpoint_counts"] != EXPECTED_SHARED_ENDPOINT_COUNTS:
        raise AssertionError("unexpected T04 shared-endpoint counts")
    if template["selected_path_shape_counts"] != EXPECTED_SELECTED_PATH_SHAPE_COUNTS:
        raise AssertionError("unexpected T04 selected-path shape counts")
    ...
    if equality_chain(equality) != expected_chain:
        raise AssertionError("unexpected F13 equality chain")
    if strict["outer_pair"] != equality["start_pair"]:
        raise AssertionError("F13 strict outer pair must start equality path")
    if strict["inner_pair"] != equality["end_pair"]:
        raise AssertionError("F13 strict inner pair must end equality path")
...
def _family_local_lemma(strict: Mapping[str, Any], equality: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "packet_name": "T04/F13 self-edge local lemma packet",
...
def t04_self_edge_lemma_packet_payload(
    self_edge_packet: dict[str, Any],
    template_catalog: dict[str, Any],
) -> dict[str, Any]:
    """Return the focused review-pending T04/F13 local lemma packet."""
    ...
    payload = {
        ...
        "interpretation": [
            "This packet isolates the T04/F13 self-edge motif from existing review-pending n=9 diagnostics.",
...
def assert_expected_t04_self_edge_lemma_packet(payload: Mapping[str, Any]) -> None:
    """Assert stable constants for the focused T04/F13 local lemma packet."""
    ...
    for key in ("row", "witness_order", "outer_pair", "inner_pair", "outer_interval", "inner_interval"):
        if primary[key] != strict[key]:
            raise AssertionError(f"F13 primary conflict {key} mismatch")
    lemma = packet["local_lemma"]
    if lemma["review_status"] != "review_pending":
        raise AssertionError("F13 local lemma review status mismatch")
