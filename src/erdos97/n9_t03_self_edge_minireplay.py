"""Minimal replay for the n=9 T03 self-edge local lemma packet.

This module treats the focused T03 packet as input data and checks only the
small local proof skeleton for its two family packets: selected-distance
...
SCHEMA = "erdos97.n9_t03_self_edge_minireplay.v1"
STATUS = "REVIEW_PENDING_T03_SELF_EDGE_MINIREPLAY"
TRUST = "REVIEW_PENDING_DIAGNOSTIC"
SOURCE_PACKET_SCHEMA = "erdos97.n9_vertex_circle_t03_self_edge_lemma_packet.v1"
...
EXPECTED_ASSIGNMENT_COUNTS = {"F05": 18, "F15": 2}
EXPECTED_STRICT_ROWS = {"F05": 6, "F15": 0}
EXPECTED_OUTER_PAIRS = {"F05": [3, 7], "F15": [1, 4]}
EXPECTED_INNER_PAIRS = {"F05": [1, 7], "F15": [3, 4]}


def replay_family(packet: dict[str, Any]) -> tuple[dict[str, object], list[str]]:
    """Replay one T03 family packet and return a compact summary plus errors."""
    errors: list[str] = []
...
def replay_packet(packet: dict[str, Any]) -> tuple[dict[str, object], list[str]]:
    ...
    if sum(assignment_counts.values()) != EXPECTED_ASSIGNMENT_COUNT:
        errors.append("family assignment counts do not sum to expected T03 count")
    ...
    if not summary["all_self_edge_contradictions"]:
        errors.append("not every T03 family replay closed a self-edge contradiction")
    return summary, errors
...
def minireplay_payload(packet: dict[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "trust": TRUST,
        "claim_scope": (
            "Minimal input-data replay of the focused T03 multi-family self-edge "
        ...
        "replay": replay,
        "interpretation": (
            "Each T03 family packet identifies the strict outer and inner chord "
...
def assert_expected_payload(payload: dict[str, Any]) -> None:
    """Assert the stable expected T03 mini-replay facts."""
    ...
    if not isinstance(family_replays, list) or len(family_replays) != 2:
