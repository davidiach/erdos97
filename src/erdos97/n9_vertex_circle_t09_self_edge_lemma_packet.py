"""Build a focused T09/F03 n=9 self-edge local lemma packet.
...
CLAIM_SCOPE = (
    "Focused T09/F03 self-edge local lemma packet for eighteen n=9 frontier "
...
EXPECTED_TEMPLATE_ID = "T09"
EXPECTED_TEMPLATE_KEY = (
    "self_edge|rows=6|strict_edges=54|conflicts=2:1:1x5,3:1:0x2,3:1:1x4,3:2:1x2"
)
EXPECTED_FAMILY_ID = "F03"
...
EXPECTED_SELF_EDGE_CONFLICT_COUNT = 13
EXPECTED_CYCLE_EDGE_COUNT = 0
EXPECTED_FAMILY_ASSIGNMENT_COUNTS = {"F03": 18}
EXPECTED_FAMILY_ORBIT_SIZES = {"F03": 18}
EXPECTED_PATH_LENGTH_COUNTS = {"6": 18}
EXPECTED_SHARED_ENDPOINT_COUNTS = {"1": 18}
EXPECTED_SELECTED_PATH_SHAPE_COUNTS = {"2:1:1:path=6": 18}
...
EXPECTED_EQUALITY_CHAINS = {
    "F03": ([1, 3], [0, 1], [0, 8], [4, 8], [3, 4], [2, 3], [1, 2]),
}
...
def _family_records_by_id(template: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if missing:
        raise ValueError(f"missing T09 families: {missing!r}")
...
def source_artifacts(
    ...
    """Return embedded source-artifact metadata for the T09 packet."""

    return [
        {
            "path": "data/certificates/n9_vertex_circle_self_edge_template_packet.json",
            "role": "source T09/F03 self-edge template record",
        ...
        },
        {
            ...
            "role": "catalog crosswalk confirming T09 coverage and shape summary",
...
def _primary_conflict(replay: dict[str, Any], strict: Mapping[str, Any]) -> dict[str, Any]:
    ...
    raise AssertionError("primary T09 self-edge conflict not found in replay")
...
def _assert_source_records(
    ...
    if template["template_key"] != EXPECTED_TEMPLATE_KEY:
        raise AssertionError("unexpected T09 template key")
    if template["status"] != "self_edge":
        raise AssertionError("T09 must remain a self-edge template")
    if template["assignment_ids"] != list(EXPECTED_ASSIGNMENT_IDS):
        raise AssertionError("unexpected T09 assignment ids")
    if template["assignment_count"] != EXPECTED_ASSIGNMENT_COUNT:
        raise AssertionError("unexpected T09 assignment count")
    if template["families"] != list(EXPECTED_FAMILY_IDS):
        raise AssertionError("unexpected T09 family list")
    if template["family_count"] != EXPECTED_FAMILY_COUNT:
        raise AssertionError("unexpected T09 family count")
    if template["orbit_size_sum"] != EXPECTED_ORBIT_SIZE_SUM:
        raise AssertionError("unexpected T09 orbit-size sum")
    if template["core_size"] != EXPECTED_CORE_SIZE:
        raise AssertionError("unexpected T09 core size")
    if template["strict_edge_count"] != EXPECTED_STRICT_EDGE_COUNT:
        raise AssertionError("unexpected T09 strict-edge count")
    if template["path_length_counts"] != EXPECTED_PATH_LENGTH_COUNTS:
        raise AssertionError("unexpected T09 path-length counts")
    if template["shared_endpoint_counts"] != EXPECTED_SHARED_ENDPOINT_COUNTS:
        raise AssertionError("unexpected T09 shared-endpoint counts")
    if template["selected_path_shape_counts"] != EXPECTED_SELECTED_PATH_SHAPE_COUNTS:
        raise AssertionError("unexpected T09 selected-path shape counts")
    ...
    if equality_chain(equality) != expected_chain:
        raise AssertionError("unexpected F03 equality chain")
    if strict["outer_pair"] != equality["start_pair"]:
        raise AssertionError("F03 strict outer pair must start equality path")
    if strict["inner_pair"] != equality["end_pair"]:
        raise AssertionError("F03 strict inner pair must end equality path")
...
def _family_local_lemma(strict: Mapping[str, Any], equality: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "packet_name": "T09/F03 self-edge local lemma packet",
...
def t09_self_edge_lemma_packet_payload(
    self_edge_packet: dict[str, Any],
    template_catalog: dict[str, Any],
) -> dict[str, Any]:
    """Return the focused review-pending T09/F03 local lemma packet."""
    ...
    payload = {
        ...
        "interpretation": [
            "This packet isolates the T09/F03 self-edge motif from existing review-pending n=9 diagnostics.",
            "The family record has six local rows that force the displayed equality chain.",
...
def assert_expected_t09_self_edge_lemma_packet(payload: Mapping[str, Any]) -> None:
    """Assert stable constants for the focused T09/F03 local lemma packet."""
    ...
    for key in ("row", "witness_order", "outer_pair", "inner_pair", "outer_interval", "inner_interval"):
        if primary[key] != strict[key]:
            raise AssertionError(f"F03 primary conflict {key} mismatch")
    lemma = packet["local_lemma"]
    if lemma["review_status"] != "review_pending":
        raise AssertionError("F03 local lemma review status mismatch")
