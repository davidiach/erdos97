"""Minimal replay for the n=9 T06/F11 self-edge local lemma packet.

This module treats the focused T06 packet as input data and checks only the
small local proof skeleton for its F11 family packet: selected-distance
...
SCHEMA = "erdos97.n9_t06_self_edge_minireplay.v1"
STATUS = "REVIEW_PENDING_T06_SELF_EDGE_MINIREPLAY"
TRUST = "REVIEW_PENDING_DIAGNOSTIC"
SOURCE_PACKET_SCHEMA = "erdos97.n9_vertex_circle_t06_self_edge_lemma_packet.v1"
SOURCE_PACKET = "data/certificates/n9_vertex_circle_t06_self_edge_lemma_packet.json"

EXPECTED_FAMILY_IDS = ["F11"]
EXPECTED_ASSIGNMENT_COUNT = 18
EXPECTED_ASSIGNMENT_COUNTS = {"F11": 18}
EXPECTED_STRICT_ROWS = {"F11": 1}
EXPECTED_OUTER_PAIRS = {"F11": [3, 8]}
EXPECTED_INNER_PAIRS = {"F11": [3, 5]}


def replay_family(packet: dict[str, Any]) -> tuple[dict[str, object], list[str]]:
    """Replay the T06/F11 family packet and return a compact summary plus errors."""
    errors: list[str] = []
...
def replay_packet(packet: dict[str, Any]) -> tuple[dict[str, object], list[str]]:
    ...
    if sum(assignment_counts.values()) != EXPECTED_ASSIGNMENT_COUNT:
        errors.append("family assignment counts do not sum to expected T06 count")
    ...
    if not summary["all_self_edge_contradictions"]:
        errors.append("not every T06 family replay closed a self-edge contradiction")
    return summary, errors
...
def minireplay_payload(packet: dict[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "replay": replay,
        "interpretation": (
            "The T06/F11 family packet identifies the strict outer and inner "
...
def assert_expected_payload(payload: dict[str, Any]) -> None:
    """Assert the stable expected T06/F11 mini-replay facts."""
