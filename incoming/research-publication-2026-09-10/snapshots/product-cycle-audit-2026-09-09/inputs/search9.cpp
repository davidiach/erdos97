...
struct S{
 ...
 uint64_t visits=0,rad=0,sh=0,met=0,pairdead=0,leaves=0,limit=25000000;
 bool abort=false;
 ...
 void dfs(State&s){if(abort||leaves)return;if(++visits>limit){abort=true;return;}array<uint32_t,M>R{};if(!radial(s,R)){++rad;return;}if(!shortcut(s,R)){++sh;return;}if(!metric(s,R)){++met;return;}if(s.depth==M){
    ...
    ++leaves;cout<<"[";for(int i=0;i<M;++i){auto&r=rows[i][s.pick[i]];if(i)cout<<",";cout<<"["<<r.t[0]<<","<<r.g[0]<<","<<r.t[1]<<","<<r.g[1]<<"]";}cout<<"]\n";return;}int best=-1;vector<int>O;for(int c=0;c<M;++c)if(s.pick[c]<0){auto q=opts(c,s);if(best<0||q.size()<O.size()){best=c;O=move(q);if(O.empty())break;}}if(O.empty()){++pairdead;return;}for(int x:O){s.pick[best]=x;++s.depth;dfs(s);--s.depth;s.pick[best]=-1;if(abort||leaves)return;}}
};
int main(){S s;State st;s.dfs(st);cerr<<"visits="<<s.visits<<" rad="<<s.rad<<" shortcut="<<s.sh<<" metric="<<s.met<<" pair="<<s.pairdead<<" leaves="<<s.leaves<<" abort="<<s.abort<<"\n";return s.leaves?0:1;}
