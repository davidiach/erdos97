...
struct S{
 ...
 void dfs(State&s){if(abort||(first&&leaves))return;if(++visits>limit){abort=true;return;}array<uint32_t,M>R{};if(!radial(s,R)){++rad;return;}if(!shortcut(s,R)){++sh;return;}if(!metric(s,R)){++met;return;}if(s.depth==M){
