...
def rot(p):return (-(p[0]+3*p[1])/2,(p[0]-p[1])/2)
def norm(p):return p[0].square()+3*p[1].square()
def sub(a,b):return a[0]-b[0],a[1]-b[1]
def cross(a,b):return a[0]*b[1]-a[1]*b[0]
def squared(a,b):return norm(sub(a,b))
def orientation(a,b,c):return cross(sub(b,a),sub(c,a))
...
def build(chain_orbits:int,bits:int):
    ...
    for j in range(chain_orbits):
        ...
        if j+1<chain_orbits:
            ...
            require(new.lo>0 and new.hi<t.lo,'strict parameter decrease not certified; increase precision')
            t=new
    return reps,ts
...
class DSU:
    ...
    def root(self,x):
        ...
        return x
    def union(self,a,b):self.p[self.root(b)]=self.root(a)

def finite_check(chain_orbits=5,bits=2048):
    reps,ts=build(chain_orbits,bits);m=len(reps)
    points=[];labels=[]
    for i,z in enumerate(reps):
        for k in range(3):points.append(z);labels.append((i,k));z=rot(z)
    n=len(points)
    def half(z):
        x,y=z
        if y.lo>0:return 0
    ...
    def compare(i,j):
        a,b=points[i],points[j];ha,hb=half(a),half(b)
    ...
    pairs=list(combinations(range(n),2));ix={p:i for i,p in enumerate(pairs)};ds=DSU(len(pairs))
    def edge(a,b):return ix[tuple(sorted((a,b)))]
    def shifted(i,k):return 3*(i//3)+(i%3+k)%3
    for a,b in pairs:
        for k in (1,2):ds.union(edge(a,b),edge(shifted(a,k),shifted(b,k)))
    for i in range(1,m):
        for k in range(3):
            source=3*i+k;own=edge(source,3*i+(k+1)%3)
            ds.union(own,edge(source,k))
            if i>=2:ds.union(own,edge(source,3*(i-1)+k))
    ...
    rows=[];separations=[]
    for a in range(n):
        groups=defaultdict(list);enclosures={}
        for b in range(n):
            if a==b:continue
            e=edge(a,b);cl=ds.root(e);groups[cl].append(b)
            if cl in enclosures:enclosures[cl]=enclosures[cl].intersect(distances[e])
            else:enclosures[cl]=distances[e]
        for c,d in combinations(groups,2):
            v,w=enclosures[c],enclosures[d]
            sep=max(v.lo-w.hi,w.lo-v.hi)
            require(sep>0,f'unproved separation at {labels[a]}: {groups[c]}, {groups[d]}; increase precision or audit extra identities')
            separations.append(sep)
        mult=max(map(len,groups.values()))
        if a//3==0:require(mult==2,'unexpected anchor multiplicity')
        elif a//3==1:require(mult==3,'unexpected initial-orbit multiplicity')
        else:require(mult>=4,'a constructed rich vertex lost its witnesses')
    ...
    bounds={'support_determinant_div_sqrt3':min(supports),'distinct_squared_distance':min(d.lo for d in distances),
    ...
    return {'status':'EXACT_FINITE_INSTANCE_VERIFIED_NOT_A_COUNTEREXAMPLE',
      ...
      'support_checks':len(supports),'distinct_pairs':len(pairs),'distance_class_separations':len(separations),
      'cyclic_order':[list(labels[v]) for v in order],
      'positive_dyadic_lower_bounds':{k:{'numerator_hex':hex(v),'denominator_power_of_two':bits} for k,v in bounds.items()},
      'rows':rows}
...
def main():
    ...
    try:
        ...
        if not args.algebra_only:result['finite']=finite_check(args.chain_orbits,args.bits)
