...
import sys
import unittest
from algebra import Poly, check_algebra, identical, s, t
from intervals import Interval as I
from verify import build, finite_check, norm, rot
...
class AlgebraTests(unittest.TestCase):
    ...
    def test_wrong_identity_rejected(self):
        with self.assertRaises(AssertionError): identical((s+t)**2,s*s+t*t,'deliberately wrong')
    ...
    def test_box_contains_exact_evaluations(self):
        ...
        lo,hi=p.box()
        for a in range(11):
            for b in range(11):self.assertTrue(lo<=p.evaluate(Q(a,100),Q(b,100))<=hi)
    def test_invalid_exponent(self):
        with self.assertRaises(ValueError): _=s**-1
    def test_scalar_division_guard(self):
        with self.assertRaises(ZeroDivisionError): _=s/0
...
class IntervalTests(unittest.TestCase):
    ...
    def test_rational_outward_rounding(self):
        ...
        for _ in range(400):
            ...
            for v,e in ((x,a),(x+y,a+b),(x-y,a-b),(x*y,a*b),(x.square(),a*a)):
                self.contains(v,e)
            if b:self.contains(x/y,a/b)
    def test_square_crossing_zero(self):
        q=I(-3,4,64).square();self.assertEqual(q.lo,0)
    ...
    def test_exact_square_root(self):
        q=I.exact(4,64).sqrt();self.assertEqual((q.lo,q.hi),(2<<64,2<<64))
    def test_zero_divisor(self):
        with self.assertRaises(ZeroDivisionError):_=I.exact(1)/I(-1,1,256)
    def test_negative_square_root(self):
        with self.assertRaises(ArithmeticError):_=I.exact(-1).sqrt()
    def test_mixed_precision(self):
        with self.assertRaises(ValueError):_=I.exact(1,64)+I.exact(1,128)
    def test_empty_intersection(self):
        with self.assertRaises(ValueError):I.exact(1).intersect(I.exact(2))
    def test_bad_precision(self):
        for b in (0,16,True):
            with self.assertRaises(ValueError):I(0,0,b)
...
class GeometricControlTests(unittest.TestCase):
    ...
    def test_precision_agreement(self):
        ...
        for k in ('cyclic_order','rows','maximum_multiplicity_distribution','distance_class_separations'):
    ...
    def test_rotation_exact_rational_control(self):
        p=(Q(-43,206),Q(97,206));q=p
        for _ in range(3):q=(-(q[0]+3*q[1])/2,(q[0]-q[1])/2)
    ...
    def test_bad_parameters(self):
        for m,b in ((0,256),(-1,256),(True,256),(1,32),(1,True)):
            with self.assertRaises(ValueError):build(m,b)
    def test_cli_failure_has_no_success_report(self):
        p=subprocess.run([sys.executable,str(ROOT/'verify.py'),'--chain-orbits','0'],capture_output=True,text=True)
    ...
    def test_cli_algebra_only(self):
        p=subprocess.run([sys.executable,str(ROOT/'verify.py'),'--algebra-only'],capture_output=True,text=True,check=True)
