...
class Poly:
    ...
    def __add__(self, value):
        other=Poly.coerce(value); result=self.terms.copy()
        for e,c in other.terms.items(): result[e]=result.get(e,Q(0))+c
        return Poly(result)
    __radd__=__add__
    def __neg__(self): return Poly({e:-c for e,c in self.terms.items()})
    def __sub__(self, value): return self + -Poly.coerce(value)
    def __rsub__(self, value): return Poly.coerce(value) + -self
    def __mul__(self, value):
        other=Poly.coerce(value); result={}
        for (a,b),c in self.terms.items():
            for (d,e),f in other.terms.items():
                k=(a+d,b+e); result[k]=result.get(k,Q(0))+c*f
        return Poly(result)
    __rmul__=__mul__
    def __truediv__(self, scalar):
        scalar=Q(scalar)
        if not scalar: raise ZeroDivisionError('zero polynomial scalar divisor')
    ...
    def __pow__(self, n):
        ...
        return result
    def dt(self): return Poly({(a,b-1):b*c for (a,b),c in self.terms.items() if b})
...
def dot(p,q): return p[0]*q[0]+3*p[1]*q[1]
def sub(p,q): return p[0]-q[0],p[1]-q[1]
def D(v): return 1+3*v*v
# z(v) = numerator(v) / (2 D(v)).
def znum(v): return -D(v)+6*v, 1-3*v*v
# q(v) = omega^2 z(v); the arc approaching 1 from below.
def qnum(v): return 2-3*v-3*v*v, 3*v*(v-1)


def check_algebra():
    ds,dt=D(s),D(t); xs,ys=znum(s);xt,yt=znum(t)
    B=1-2*t+3*t*t; f=s*s-B*s+t*t
    ...
    for k in range(3):
        ...
        p=rot(p)
    identical((3*(dt-3*t)).dt()*dt-3*(dt-3*t)*dt.dt(), -9*(1-3*t*t), 'anchor M derivative')
    ...
    for eps in (1,-1):
        for k in range(3):
            u,v=qnum(t); v=eps*v
            for _ in range(k): u,v=rot((u,v))
            delta=(x*dt-u*ds,y*dt-v*ds)
            identical(dot(normal,delta),12*ds*P[eps,k],f'support margin {eps},{k}')
            supports.append({'reflection':eps,'rotation':k,'normalized_polynomial':P[eps,k].serialized()})
    identical(P[-1,0],3*(s*s*(2-3*t)+2*s*t*(1-3*t)+t*t+t+3*s*s*t*t)/2,'positive cross-arc decomposition')
    ...
    a=Q(1,10); seed=(Q(-43,206),Q(97,206))
    ...
    for eps in (1,-1):
        for k in range(3):
            ...
            identical(derivative,factored_derivatives[eps,k],f'seed derivative factorization {eps},{k}')
    ...
    require(all(limits[i+1]-limits[i]>Q(1,2) for i in range(2)),'disjoint quarter-width bands')
    ...
    return {'status':'EXACT_ALGEBRA_CHECKS_PASSED_NOT_FORMAL_GEOMETRIC_PROOF',
      'support_polynomials':supports,'support_uniform_lower_bounds':{str(k):str(v) for k,v in lower.items()},
      'squared_support_factor_lower_bound':str(Q(97,100)**2),
      'seed_distance_functions':seed_records,'seed_band_perturbation_bound':str(perturbation),
      'first_iterate_below':'1/80','arbitrary_length_theorem':'paper proof in README.md; independent review pending'}
