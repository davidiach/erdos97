...
class Interval:
    ...
    def exact(value,bits=256):
        q=Fraction(value);scale=1<<bits
        return Interval(q.numerator*scale//q.denominator,-((-q.numerator*scale)//q.denominator),bits)
    def coerce(self,x):
        if not isinstance(x,Interval):return Interval.exact(x,self.bits)
        if x.bits!=self.bits:raise ValueError('mixed interval precisions')
        return x
    def __add__(self,x):
        x=self.coerce(x);return Interval(self.lo+x.lo,self.hi+x.hi,self.bits)
    __radd__=__add__
    def __neg__(self):return Interval(-self.hi,-self.lo,self.bits)
    ...
    def __truediv__(self,x):
        ...
        lo,hi=min(v),max(v)
        return Interval(lo.numerator//lo.denominator,-((-hi.numerator)//hi.denominator),self.bits)
    ...
    def sqrt(self):
        if self.lo<0:raise ArithmeticError('square root not certified nonnegative')
        scale=1<<self.bits;lo=isqrt(self.lo*scale);hi=isqrt(self.hi*scale)
