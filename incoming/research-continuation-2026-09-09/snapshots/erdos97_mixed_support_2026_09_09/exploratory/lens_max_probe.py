...
def run(b,c):
 ...
 levels={0.0}
 for f in polys:
  ts=[-c,c]+[x for x in roots([4,0,2*f[2],f[3]]) if -c<=x<=c]
 ...
 for r in [(u+v)/2 for u,v in zip(ls,ls[1:])]+ls:
  ...
  for i,f in enumerate(polys):
   ff=f.copy();ff[-1]-=r
   for t in roots(ff):
    if -c-1e-7<t<c+1e-7 and not(i==0 and abs(t-c)<1e-7):points.append([i,t])
...
Path(__file__).with_suffix('.json').write_text(json.dumps({'status':'floating exploratory only','bad_to_maximum_absolute_x_claim':bad},indent=2))
