...
int main(int argc,char**argv){
 try{
  if(argc!=2)throw std::runtime_error("usage: check_circle_metric matrix.txt");
  std::ifstream f(argv[1]);int n;Z scale,radius;
  if(!(f>>n>>scale>>radius)||n!=138||scale<=0||radius<=0)throw std::runtime_error("invalid header");
  ...
  unsigned long long equalities=0,triangles=0,quads=0,kalmanson=0,ptolemy=0,ptolemy_equalities=0;
  ...
  for(int i=0;i<n;i++)for(int j=i+1;j<n;j++)for(int k=j+1;k<n;k++)for(int l=k+1;l<n;l++){
   ...
   if(x+y<z||x+z<y||y+z<x)throw std::runtime_error("Ptolemy failure");
   unsigned long long eq=(x+y==z)+(x+z==y)+(y+z==x);
   bool within=(l<74)||(i>=74);
   if(eq!=(within?1ULL:0ULL))throw std::runtime_error("unexpected Ptolemy equality pattern");
   ptolemy_equalities+=eq;
   ptolemy+=3;quads++;
  }
  ...
  if(n!=138)throw std::runtime_error("this audit requires the specified 138-label control");
  const int idx[3]={1,74,75};
  for(int i=0;i<3;i++)for(int j=0;j<3;j++)G[i][j]=d[0][idx[i]]*d[0][idx[i]]+d[0][idx[j]]*d[0][idx[j]]-d[idx[i]][idx[j]]*d[idx[i]][idx[j]];
