...
import scripts.check_n9_high_risk_frontier_packet as checker
...
def test_high_risk_frontier_cli_lightweight_skips_regeneration(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    def fail_build() -> dict[str, object]:
        raise AssertionError("lightweight check should not regenerate")

    monkeypatch.setattr(checker, "build_payload", fail_build)

    assert checker.main(["--check", "--lightweight", "--json"]) == 0
