...
from erdos97.n9_vertex_circle_t07_self_edge_lemma_packet import (
    assert_expected_t07_self_edge_lemma_packet,
)
from scripts.check_n9_vertex_circle_t07_self_edge_lemma_packet import (
    DEFAULT_ARTIFACT,
    load_artifact,
...
def _template_record(source_payloads: dict[str, object]) -> dict[str, object]:
    for template in source_payloads["self_edge_packet"]["templates"]:  # type: ignore[index]
        if template["template_id"] == "T07":
            return template
    raise AssertionError("missing T07 template")


def test_t07_self_edge_lemma_packet_counts_and_scope() -> None:
    payload = load_artifact(DEFAULT_ARTIFACT)

    assert_expected_t07_self_edge_lemma_packet(payload)
    ...
    assert payload["template_id"] == "T07"
    assert payload["template_key"] == "self_edge|rows=5|strict_edges=45|conflicts=2:1:1x3,3:1:0x1,3:1:1x1,3:2:1x1"
    ...
    assert payload["family_ids"] == ["F06"]
    assert payload["family_assignment_counts"] == {"F06": 18}
    assert payload["family_orbit_sizes"] == {"F06": 18}
...
def test_t07_self_edge_lemma_packet_records_expected_local_shape() -> None:
    payload = load_artifact(DEFAULT_ARTIFACT)
    packet = _family_packet(payload)

    assert packet["family_id"] == "F06"
    ...
    assert packet["strict_inequality"]["witness_order"] == [1, 2, 4, 7]  # type: ignore[index]
    assert packet["strict_inequality"]["outer_pair"] == [1, 4]  # type: ignore[index]
    ...
    assert packet["equality_chain"] == [[1, 4], [4, 5], [5, 6], [2, 6], [1, 2]]
    ...
    assert packet["replay"]["self_edge_conflict_count"] == 6  # type: ignore[index]
...
def test_t07_self_edge_lemma_packet_checker_passes_lightweight() -> None:
    payload = load_artifact(DEFAULT_ARTIFACT)
...
def test_t07_self_edge_lemma_packet_rejects_tampered_equality_path() -> None:
    ...
    assert any("F06 distance_equality mismatch" in error for error in errors)
    assert any("expected T07 self-edge lemma packet" in error for error in errors)


def test_t07_self_edge_lemma_packet_rejects_tampered_strict_row() -> None:
    payload = load_artifact(DEFAULT_ARTIFACT)
    packet = _family_packet(payload)
    ...
    assert any("F06 strict_inequality mismatch" in error for error in errors)


def test_t07_self_edge_lemma_packet_rejects_wrong_conflict_count() -> None:
    payload = load_artifact(DEFAULT_ARTIFACT)
    packet = _family_packet(payload)
    ...
    assert any("F06 replay self_edge_conflict_count mismatch" in error for error in errors)


def test_t07_self_edge_lemma_packet_rejects_missing_no_proof_note() -> None:
    payload = load_artifact(DEFAULT_ARTIFACT)
...
def test_t07_self_edge_lemma_packet_detects_source_packet_mismatch() -> None:
    payload = load_artifact(DEFAULT_ARTIFACT)
    ...
    assert any(
        ...
        or "source-bound T07 self-edge lemma packet failed" in error
        for error in errors
    )
