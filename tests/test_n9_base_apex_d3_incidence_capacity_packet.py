...
def test_full_packet_nonclaiming_wording_and_unknown_states() -> None:
    payload = expected_packet_payload()
...
def test_full_packet_rows_are_r000_through_r087() -> None:
    payload = expected_packet_payload()
    rows = payload["rows"]
    ...
    assert [row["profile_ledger_id"] for row in rows[::8]] == [
        f"P{index:02d}" for index in range(19, 30)
    ]
    assert rows[0]["profile_ledger_id"] == "P19"
    assert rows[-1]["profile_ledger_id"] == "P29"


def test_full_packet_target_capacity_totals_are_pinned() -> None:
    payload = expected_packet_payload()
    rows = payload["rows"]
    ...
    assert rows[0]["target_capacity_totals_by_cyclic_length"] == {
        "1": 9,
        "2": 16,
        "3": 17,
        "4": 18,
    }
    assert rows[-1]["target_capacity_totals_by_cyclic_length"] == {
        "1": 9,
        "2": 15,
        "3": 18,
        "4": 18,
    }
    assert all(row["target_capacity_total"] == 60 for row in rows)
    assert all(
...
def test_full_packet_deficient_base_chords_are_pinned() -> None:
    payload = expected_packet_payload()
    ...
    assert payload["rows"][-1]["deficient_base_chords"] == [
        {"cyclic_length": 2, "base_index": 0, "chord": [0, 2]},
        {"cyclic_length": 2, "base_index": 2, "chord": [2, 4]},
...
def test_full_packet_profile_option_counts_are_pinned() -> None:
    payload = expected_packet_payload()

    assert (
        payload["profile_option_counts_by_excess"]
        == EXPECTED_PROFILE_OPTION_COUNTS_BY_EXCESS
    )
    assert payload["rows"][0]["profile_option_counts"] == {
        "0": EXPECTED_PROFILE_OPTION_COUNTS_BY_EXCESS["0"],
        "6": EXPECTED_PROFILE_OPTION_COUNTS_BY_EXCESS["6"],
    }
    assert payload["rows"][-1]["profile_option_counts"] == {
        "0": EXPECTED_PROFILE_OPTION_COUNTS_BY_EXCESS["0"],
        "1": EXPECTED_PROFILE_OPTION_COUNTS_BY_EXCESS["1"],
    }


def test_full_packet_payload_passes_independent_checker() -> None:
    payload = expected_packet_payload()
...
def test_full_packet_summary_is_nonclaiming() -> None:
    payload = expected_packet_payload()
    errors = validate_payload(payload)
...
def test_full_packet_checker_rejects_tampered_claim_scope() -> None:
    payload = expected_packet_payload()
    payload["claim_scope"] = (
        "Focused D=3 packet; not a proof and not a global status update."
    )
...
def test_full_packet_checker_rejects_tampered_provenance() -> None:
    payload = expected_packet_payload()
...
def test_full_packet_checker_rejects_tampered_source_artifacts() -> None:
    payload = expected_packet_payload()
...
def test_full_packet_checker_rejects_drifted_d3_packet_source(monkeypatch) -> None:
    payload = expected_packet_payload()
    source_path = (
...
def test_full_packet_checker_rejects_bool_top_level_int() -> None:
    payload = expected_packet_payload()
    payload["n"] = True
...
def test_full_packet_checker_rejects_bool_nested_int() -> None:
    payload = expected_packet_payload()
...
def test_full_packet_checker_rejects_representative_reordering() -> None:
    payload = expected_packet_payload()
    ...
    assert any("rows must be ordered R000..R087" in error for error in errors)
    assert any("payload.rows[0].representative_id" in error for error in errors)


def test_full_packet_checker_rejects_malformed_row_without_crashing() -> None:
    payload = expected_packet_payload()
...
def test_full_packet_checker_rejects_malformed_multiset_without_crashing() -> None:
    payload = expected_packet_payload()
    payload["rows"][0]["excess_multiset"] = 6

    errors = validate_payload(payload)

    assert any("excess_multiset must be a list of ints" in error for error in errors)
    assert any("source join skipped malformed payload row 0" in error for error in errors)


def test_full_packet_checker_rejects_unknown_profile_excess_without_crashing() -> None:
    payload = expected_packet_payload()
    bad_profile = [-1, 0, 0, 0, 0, 0, 0, 0, 7]
    payload["rows"][0]["representative_profile_sequence"] = bad_profile
    payload["rows"][0]["excess_multiset"] = bad_profile

    errors = validate_payload(payload)

    assert any("contains unknown profile excess" in error for error in errors)
    assert any("source is missing row 0" in error for error in errors)


def test_full_packet_checker_rejects_malformed_escape_without_crashing() -> None:
    payload = expected_packet_payload()
...
def test_full_packet_checker_cli_json(tmp_path: Path) -> None:
    artifact = tmp_path / "n9_base_apex_d3_incidence_capacity_packet.json"
    artifact.write_text(
...
def test_full_packet_analyzer_cli_json() -> None:
    result = subprocess.run(
        [
            sys.executable,
