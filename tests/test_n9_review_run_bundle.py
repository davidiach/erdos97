...
def test_n9_review_run_bundle_payload_contains_expected_layers() -> None:
    ...
    assert len(bundle["routes"]) == 6
    assert len(bundle["command_records"]) == 22
    assert len(bundle["review_gates"]) == 8
    assert len(bundle["infrastructure_gates"]) == 2
    assert len(bundle["evidence_records"]) == 22
...
def test_n9_review_run_bundle_summary_is_static_by_default() -> None:
    ...
    assert summary["command_count"] == 22
    assert summary["evidence_record_count"] == 22
