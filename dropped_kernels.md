...
- `erdos97-search --verify data/runs/archive_C12_offsets_4_5_8_11_near_convex.json --tol 1e-6` reports `ok_at_tol=false` with max selected-distance spread about `1.85e-6`; its empirical multiplicities are `[4,3,4,3,...]`, so half the centers achieve only `E=3` even numerically. It is excluded from `STATE.md`.
- `erdos97-search --verify data/runs/archive_C12_offsets_2_3_4_10_degenerate.json --tol 1e-6` reports `ok_at_tol=false` because convexity/minimum-distance diagnostics collapse. The full normalized JSON is retained only to preserve the imported artifact shape; mathematically it reduces to a collapsed three-point configuration and is excluded from `STATE.md`.
